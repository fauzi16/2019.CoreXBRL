...
public final class DimensionTest {
    ...
    public static void setUp() {
        try {
            ...
            tstTaxonomy = TestHelper.getDTS("xbrl/test/taxonomy_typed_dimension/t-st-2006-12-31.xsd");
        } catch (Exception ex) {
    ...
    public void dimension() {
        ...
        Concept dimensionKontinent = tstTaxonomy.getConceptByID("t-st_KontinentDimension");
        Concept dimensionLaender = tstTaxonomy.getConceptByID("d-la_LaenderDimension");
        Concept dimensionAnzahl = tstTaxonomy.getConceptByID("t-st_AnzahlDimension");

        assertNotNull(dimensionKontinent);
        assertNotNull(dimensionLaender);
        assertNotNull(dimensionAnzahl);

        Dimension kontinentDimension = hcEuropa.getDimension(dimensionKontinent, "http://www.xbrl-supervision.org/test/t-st/hcEuropa");
        Dimension laenderDimension = hcEuropa.getDimension(dimensionLaender, "http://www.xbrl-supervision.org/test/t-st/hcEuropa");
        Dimension anzahlDimension = hcEuropa.getDimension(dimensionAnzahl, "http://www.xbrl-supervision.org/test/t-st/hcEuropa");
    ...
    public void dimensionAllowed() {
        try {
            ...
            assertTrue(defLinkbase.dimensionAllowed(primaryItem, mdt,"http://www.xbrl-supervision.org/test/t-st/hcEuropa","scenario"));
        } catch (Exception ex) {
            System.out.println(ex.toString());
            ex.printStackTrace();
            fail(ex.toString());
